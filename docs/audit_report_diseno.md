# Audit Report · variante diseno

> Generado el 2026-06-17T17:25:15.431129+00:00 por el motor `quality_engine 0.2.0-s6x`.
...
| Variante EIR | `diseno` |
| Versión EIR | `0.2` |
| Fuente EIR | `diseno@0.2` |
| YAMLs usados | `PBSA_v0.2_comun.yaml, PBSA_v0.2_diseno.yaml` |
...
| `H3.DOOR.QTO_Area` | D2 | `yaml_python` | ✗ FAIL | 0.00% | 95% | 60% | 0/5 instancias de IfcDoor declaran Qto_DoorBaseQuantities.Area (0.0%). |
...
| `H3.SLAB.QTO_GrossArea` | D2 | `yaml_python` | ✗ FAIL | 0.00% | 95% | 60% | 0/4 instancias de IfcSlab declaran Qto_SlabBaseQuantities.GrossArea (0.0%). |
...
| `H3.WALL.QTO_NetVolume` | D2 | `yaml_python` | ✗ FAIL | 0.00% | 95% | 60% | 0/13 instancias de IfcWallStandardCase declaran Qto_WallBaseQuantities.NetVolume (0.0%). |
...
| `H3.WINDOW.QTO_Area` | D2 | `yaml_python` | ✗ FAIL | 0.00% | 95% | 60% | 0/11 instancias de IfcWindow declaran Qto_WindowBaseQuantities.Area (0.0%). |
...
| `C-Q-03` | D8 | `yaml_python` | — N/A | — | 100% | — | Sin Qto_WallBaseQuantities.NetVolume declarado en ningún IfcWall. Sniff coste 5D no apl... |

## 6. Hallazgos clave

### 6.1 Fallos críticos (17)
...
#### `H3.WALL.QTO_NetVolume` · D2 · backend `yaml_python`

- **Estado:** ✗ FAIL
- **Score:** 0.00%
- **Mensaje:** 0/13 instancias de IfcWallStandardCase declaran Qto_WallBaseQuantities.NetVolume (0.0%).
- **Evidencia:**
```json
{
  "ifc_type": "IfcWallStandardCase",
...
#### `H3.SLAB.QTO_GrossArea` · D2 · backend `yaml_python`

- **Estado:** ✗ FAIL
- **Score:** 0.00%
- **Mensaje:** 0/4 instancias de IfcSlab declaran Qto_SlabBaseQuantities.GrossArea (0.0%).
- **Evidencia:**
```json
{
  "ifc_type": "IfcSlab",
...
#### `H3.DOOR.QTO_Area` · D2 · backend `yaml_python`

- **Estado:** ✗ FAIL
- **Score:** 0.00%
- **Mensaje:** 0/5 instancias de IfcDoor declaran Qto_DoorBaseQuantities.Area (0.0%).
- **Evidencia:**
```json
{
  "ifc_type": "IfcDoor",
...
#### `H3.WINDOW.QTO_Area` · D2 · backend `yaml_python`

- **Estado:** ✗ FAIL
- **Score:** 0.00%
- **Mensaje:** 0/11 instancias de IfcWindow declaran Qto_WindowBaseQuantities.Area (0.0%).
- **Evidencia:**
```json
{
  "ifc_type": "IfcWindow",
...
- `C-Q-03` (D8) · Sin Qto_WallBaseQuantities.NetVolume declarado en ningún IfcWall. Sniff coste 5D no aplicable.
...
11. **`H3.WALL.QTO_NetVolume` (D2):** 0/13 instancias de IfcWallStandardCase declaran Qto_WallBaseQuantities.
12. **`H3.SLAB.QTO_GrossArea` (D2):** 0/4 instancias de IfcSlab declaran Qto_SlabBaseQuantities.
13. **`H3.DOOR.QTO_Area` (D2):** 0/5 instancias de IfcDoor declaran Qto_DoorBaseQuantities.
14. **`H3.WINDOW.QTO_Area` (D2):** 0/11 instancias de IfcWindow declaran Qto_WindowBaseQuantities.
15. **`C-P-01` (D2):** IDS spec 'C-P-01' FAIL · 0/1 entidades cumplen (0%).
16. **`C-P-02` (D2):** IDS spec 'C-P-02' FAIL · 0/4 entidades cumplen (0%).
17. **`C-P-03` (D2):** IDS spec 'C-P-03' PARTIAL · 3/5 entidades cumplen (60%).
18. **`STR.BSDD.Classification` (D6):** Legacy structural check_bsdd_classification → fail.

---

**Trazabilidad:** este reporte fue generado por `scripts/s6x_generate_e6_outputs.py` a partir de `out/AC20-FZK-Haus_compliance_post_diseno_v2.json`. La matriz JSON es la fuente canónica; este Markdown es derivado.
