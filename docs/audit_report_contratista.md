# Audit Report · variante contratista

> Generado el 2026-06-17T17:25:16.016034+00:00 por el motor `quality_engine 0.2.0-s6x`.

## 1. Metadatos de auditoría

| Campo | Valor |
|---|---|
| Variante EIR | `contratista` |
| Versión EIR | `0.2` |
| Fuente EIR | `contratista@0.2` |
| YAMLs usados | `PBSA_v0.2_comun.yaml, PBSA_v0.2_contratista.yaml` |
...
| `H3.SLAB.WBS_Reference` | D2 | `yaml_python` | ✗ FAIL | 0.00% | 95% | 60% | 0/4 instancias de IfcSlab declaran Pset_SlabCommon.Reference (0.0%). |
| `H3.WALL.AcousticRating` | D2 | `yaml_python` | ✗ FAIL | 0.00% | 95% | 60% | 0/13 instancias de IfcWallStandardCase declaran Pset_WallCommon.AcousticRating (0.0%). |
...
| `H3.WALL.WBS_Reference` | D2 | `yaml_python` | ✗ FAIL | 0.00% | 95% | 60% | 0/13 instancias de IfcWallStandardCase declaran Pset_WallCommon.Reference (0.0%). |
...
#### `H3.WALL.WBS_Reference` · D2 · backend `yaml_python`

- **Estado:** ✗ FAIL
- **Score:** 0.00%
- **Mensaje:** 0/13 instancias de IfcWallStandardCase declaran Pset_WallCommon.Reference (0.0%).
- **Evidencia:**
```json
{
  "ifc_type": "IfcWallStandardCase",
...
#### `H3.SLAB.WBS_Reference` · D2 · backend `yaml_python`

- **Estado:** ✗ FAIL
- **Score:** 0.00%
- **Mensaje:** 0/4 instancias de IfcSlab declaran Pset_SlabCommon.Reference (0.0%).
- **Evidencia:**
```json
{
  "ifc_type": "IfcSlab",
...
#### `H3.WALL.AcousticRating` · D2 · backend `yaml_python`

- **Estado:** ✗ FAIL
- **Score:** 0.00%
- **Mensaje:** 0/13 instancias de IfcWallStandardCase declaran Pset_WallCommon.AcousticRating (0.0%).
- **Evidencia:**
```json
{
  "ifc_type": "IfcWallStandardCase",
...
11. **`H3.WALL.WBS_Reference` (D2):** 0/13 instancias de IfcWallStandardCase declaran Pset_WallCommon.
12. **`H3.SLAB.WBS_Reference` (D2):** 0/4 instancias de IfcSlab declaran Pset_SlabCommon.
13. **`H3.WALL.AcousticRating` (D2):** 0/13 instancias de IfcWallStandardCase declaran Pset_WallCommon.
...
**Trazabilidad:** este reporte fue generado por `scripts/s6x_generate_e6_outputs.py` a partir de `out/AC20-FZK-Haus_compliance_post_contratista_v2.json`. La matriz JSON es la fuente canónica; este Markdown es derivado.
